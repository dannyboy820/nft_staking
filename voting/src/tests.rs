...
mod tests {
    ...
    fn poll_not_found() {
        ...
        let res = query(&deps, mock_env(), QueryMsg::Poll { poll_id: 1 });
    ...
    fn happy_days_create_poll() {
        ...
        let handle_res = handle(&mut deps, env.clone(), info.clone(), msg.clone()).unwrap();
    ...
    fn happy_days_end_poll() {
        ...
        let handle_res = handle(&mut deps, creator_env.clone(), creator_info.clone(), msg).unwrap();
        ...
        let handle_res = handle(&mut deps, env.clone(), info.clone(), msg).unwrap();
    ...
    fn end_poll_zero_quorum() {
        ...
        let handle_res = handle(&mut deps, env.clone(), info, msg).unwrap();
        ...
        let res = query(&deps, env.clone(), QueryMsg::Poll { poll_id: 1 }).unwrap();
        let value: PollResponse = from_binary(&res).unwrap();
    ...
    fn end_poll_nay_rejected() {
        ...
        let handle_res = handle(&mut deps, mock_env(), info, msg.clone()).unwrap();
    ...
    fn fails_cast_vote_twice() {
        ...
        let quorum_percentage = 30;
        let msg = create_poll_msg(quorum_percentage, "test".to_string(), None, None);
        let handle_res = handle(&mut deps, env.clone(), info, msg.clone()).unwrap();
    ...
    fn fails_insufficient_funds() {
        ...
        let init_res = init(&mut deps, mock_env(), info, msg).unwrap();
